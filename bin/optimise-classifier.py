...
def optimise_lightgbm(
    ...
    logloss_scorer = make_scorer(log_loss, greater_is_better=False, needs_proba=True, labels=sorted(Y_train.unique()))
    ...
    search_spaces = {
        ...
        'max_depth': Integer(5, 80),                         # Maximum tree depth for base learners, <=0 means no limit
        'min_child_samples': Integer(20, 256),               # Minimal number of data in one leaf
        'max_bin': Integer(10, 500),                         # Max number of bins that feature values will be bucketed
        ...
        'min_child_weight': Real(0.0001, 10.0, 'uniform'),   # Minimum sum of instance weight (hessian) needed in a child (leaf)
