...
"""
Evaluate the cluster structure of the integrated data using the balanced Normalized Mutual Information (bNMI).
...
Usage:
    metric-bNMI.py --dataset=<str> --method=<str> --integration=<str> --out-file=<path> [options] <file>
...
def calculate_bNMI(adata):
    """
    Calculate the balanced Normalized Mutual Information (bNMI) score for an integrated dataset.
    ...
    Returns
    -------
    The bNMI score [0, 1] where 0 is no match between the pair of labels, and 1 a perfect match.
    """
...
def bNMI(adata, label_key, cluster_key):
    """
    Balanced Normalized Mutual Information (bNMI) for an AnnData object.
    A wrapper around `balanced_clustering.balanced_v_measure()` with the interface expected by `scib.metrics.cluster_optimal_resolution`.
