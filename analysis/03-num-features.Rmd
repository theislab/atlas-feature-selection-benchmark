...
## Number of features means and SDs

```{r extended-features-means-sds, fig.width = 8, out.width = "100%"}
sd_limits <- c(
    min(method_means$SD, dataset_means$SD),
    max(method_means$SD, dataset_means$SD)
)

datasets_means <- ggplot(
    mutate(dataset_means, Dataset = str_replace(Dataset, "\n", " ")),
    aes(x = SelFeatures, y = Dataset, colour = StandardValue)
) +
    geom_point(shape = "square") +
    colorspace::scale_colour_continuous_diverging(
        palette = "Purple-Green",
    ...
    theme(
        strip.text = element_text(
            size = 6, colour = "white", margin = margin(4.4, 4.4, 4.4, 4.4)
        ),
...
datasets_sds <- ggplot(
    mutate(dataset_means, Dataset = str_replace(Dataset, "\n", " ")),
    aes(x = SelFeatures, y = Dataset, colour = SD)
) +
    geom_point(shape = "square") +
    scale_colour_viridis_c(option = "cividis", limits = sd_limits) +
    facet_grid(. ~ Type) +
    labs(
        x = "Number of selected features",
        colour = "Standard deviation of\nstandardised values"
    ) +
    guides(
        colour = guide_colourbar(order = 1),
        size = guide_legend(
...
methods_means <- ggplot(
    method_means,
    aes(x = SelFeatures, y = Method, colour = StandardValue)
) +
    geom_point(shape = "square") +
    colorspace::scale_colour_continuous_diverging(
        palette = "Purple-Green",
...
methods_sds <- ggplot(
    method_means,
    aes(x = SelFeatures, y = Method, colour = SD)
) +
    geom_point(shape = "square") +
    scale_colour_viridis_c(option = "cividis", limits = sd_limits) +
    facet_grid(. ~ Type) +
...
num_features_means_sds_plot <- wrap_plots(
    datasets_means, datasets_sds, methods_means, methods_sds,
    ncol = 1,
    heights = c(1, 1, 1, 1),
    guides = "collect"
) &
...
## Number of features by dataset

```{r supplementary-datasets, fig.width = 8, out.width = "100%"}
dataset_features_plot <- ggplot(
    mutate(metrics_summary_plotting, Dataset = str_replace(Dataset, "\n", " "))
) +
    geom_vline(xintercept = 0, colour = "red") +
    ggforce::geom_sina(
        aes(x = StandardValue, y = Dataset),
        size = 0.5, alpha = 0.3
    ) +
    geom_point(
        data = mutate(dataset_means, Dataset = str_replace(Dataset, "\n", " ")),
        aes(x = StandardValue, y = Dataset, fill = StandardValue ),
    ...
    facet_grid(SelFeatures ~ Type, scales = "free_y", space = "free_y") +
    labs(
        title = "Standardised values by dataset",
...
dataset_features_plot
```

## Number of features by method

```{r supplementary-methods, fig.width = 8, out.width = "100%"}
method_features_plot <- ggplot(metrics_summary_plotting) +
    geom_vline(xintercept = 0, colour = "red") +
    ggforce::geom_sina(
        aes(x = StandardValue, y = Method),
        size = 0.5, alpha = 0.3
    ) +
    geom_point(
        data = method_means,
        aes(x = StandardValue, y = Method, fill = StandardValue ),
        shape = "circle filled", size = 2
    ) +
    colorspace::scale_fill_continuous_diverging(
        palette = "Purple-Green",
    ...
    facet_grid(SelFeatures ~ Type, scales = "free_y", space = "free_y") +
    labs(
        title = "Standardised values by method",
        x = "Standardised value",
        fill = "Mean feature\ncorrelation"
    ) +
    theme_features_pub() +
...
save_figure_files(
    num_features_means_sds_plot,
    here::here("analysis", "output", "num-features-means-sds"),
    width = 7, height = 6
)

save_figure_files(
    dataset_features_plot,
    here::here("analysis", "output", "dataset-num-features"),
    height = 8
)

save_figure_files(
    method_features_plot,
    here::here("analysis", "output", "method-num-features"),
    height = 8
)
```
