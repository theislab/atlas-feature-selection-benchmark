...
metrics_ranks_plotting <- metrics_ranks |>
    mutate(
        ...
        Type = factor(
            ...
            fct_recode(
                "Int. (Batch)" = "IntegrationBatch",
                "Int. (Bio)" = "IntegrationBio"
            ),
        Dataset = factor(
            Dataset,
...
label_milo_plotting <- label_milo |>
    mutate(
        ...
        Label = factor(
            Label,
            levels = labels_order,
            labels = stringr::str_wrap(labels_order, 16)
        )
    )
...
milo_plot <- ggplot(
    ...
    theme(
        axis.text.x = element_text(
            angle = 90, hjust = 1, vjust = 0.5, lineheight = 0.8
        ),
        axis.title = element_blank(),
        panel.grid = element_blank()
    )
...
markers_overlap_means <- ggplot(
    marker_overlap_means_plotting |>
        ...
        mutate(
            Lineage = case_when(
                Lineage == "Epithelial" ~ "Epi.",
                Lineage == "Endothelial" ~ "Endo",
                Lineage == "Immune" ~ "Imm.",
                Lineage == "Stroma" ~ "Strom."
            )
        ),
    aes(x = Dataset, y = Method, colour = MeanProp)
) +
    geom_point(shape = "square", size = 1.5) +
    facet_wrap(~ Lineage, nrow = 1) +
    scale_y_discrete(drop = FALSE) +
    scale_colour_viridis_c() +
    labs(colour = "Mean proportion of markers") +
    theme_features_pub() +
    theme(
        legend.position = "bottom",
        axis.title = element_blank(),
        ...
        strip.text = element_text(size = 4, colour = "white"),
        plot.margin = margin(0, 0.5, 0, 0.5)
    )

markers_overlap_sds <- ggplot(
    marker_overlap_means_plotting |>
        mutate(
            Lineage = case_when(
                Lineage == "Epithelial" ~ "Epi.",
    ...
    aes(x = Dataset, y = Method, colour = SDProp)
) +
    geom_point(shape = "square", size = 1.5) +
    facet_wrap(~ Lineage, nrow = 1) +
    scale_y_discrete(drop = FALSE) +
    scale_colour_viridis_c(option = "cividis") +
    labs(colour = "SD proportion of markers") +
    theme_features_pub() +
    theme(
        legend.position = "bottom",
        axis.title = element_blank(),
...
markers_overlap_means_sds_plot <- wrap_plots(
    markers_overlap_means, markers_overlap_sds,
    nrow = 1,
    widths = c(1, 1),
    guides = "collect"
) &
...
marker_overlaps_plotting <- marker_overlaps |>
    ...
    mutate(
        ...
        Lineage = if_else(Lineage == "Endothelial", "Endo.", Lineage)
    )

markers_overlap_celltypes_full <- marker_overlaps_plotting |>
    filter(Dataset == "Full") |>
    ggplot() +
...
markers_overlap_celltypes_immune <- marker_overlaps_plotting |>
    filter(Dataset == "Immune") |>
    ggplot() +
    geom_tile(aes(x = Label, y = Method, fill = Prop)) +
    scale_fill_viridis_c(option = "plasma", limits = c(0, 1)) +
    scale_x_discrete(expand = c(0, 0)) +
...
markers_overlap_celltypes_epi <- marker_overlaps_plotting |>
    filter(Dataset == "Epithelial") |>
    ggplot() +
    geom_tile(aes(x = Label, y = Method, fill = Prop)) +
    scale_fill_viridis_c(option = "plasma", limits = c(0, 1)) +
    scale_x_discrete(expand = c(0, 0)) +
...
markers_overlap_celltypes_plot <- wrap_plots(
    markers_overlap_celltypes_full,
    markers_overlap_celltypes_immune,
    markers_overlap_celltypes_epi,
    nrow = 1,
    widths = c(1, 1, 1),
    guides = "collect"
) &
...
markers_overlap_extended <- wrap_plots(
    markers_overlap_means_sds_plot, markers_overlap_celltypes_plot,
    nrow = 1,
    widths = c(1, 3)
)

markers_overlap_extended
```
...
save_figure_files(
    markers_overlap_extended,
    here::here("analysis", "output", "markers-overlap-extended"),
    width = 7, height = 4
)
```
