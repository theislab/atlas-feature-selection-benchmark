...
overall_values_figure <- ggplot(
    mutate(
        ...
        Type = fct_recode(
            Type,
            "Int. (Batch)" = "Integration (Batch)",
            "Int. (Bio)" = "Integration (Bio)",
            "Unseen" = "Unseen populations"
        )
    ),
    ...
    ggforce::geom_sina(
        size = 0.8, alpha = 0.5, shape = "circle filled", colour  = "white"
    ) +
    geom_point(
        data = mutate(
            metric_means,
        ...
        aes(fill = Type),
        shape = "diamond filled", size = 1.2, colour = "white"
    ) +
    scale_colour_manual(values = types_palette) +
    scale_fill_manual(values = types_palette) +
    scale_alpha_manual(values = c(0, 0.3)) +
...
mean_overlaps_plot <- ggplot(
    ...
    geom_point(shape = "square filled", stroke = 0.3) +
    scale_fill_viridis_c(option = "plasma", limits = c(0, 1)) +
    scale_colour_manual(values = c("black", "white")) +
    scale_size_continuous(
        trans = "reverse",
        range = c(0.1, 1.8), breaks = scales::breaks_extended(n = 3)
    ) +
    coord_equal() +
    labs(
...
num_methods_plot <- ggplot(
    ...
    geom_point(size = 1, shape = "circle filled") +
...
num_selected_plot <- ggplot(
    ...
    geom_point(
        aes(fill = Dataset),
        position = position_jitter(width = 0, height = 0.2, seed = 1),
        size = 1, shape = "circle filled", alpha = 0.8, colour = "grey20"
    ) +
    geom_point(
...
integration_ranks_plot <- ggplot(
    ...
    scale_size_continuous(
        ...
        range = c(0.1, 2), breaks = scales::breaks_extended(n = 6)
    ) +
    scale_alpha_continuous(
        limits = c(length(unique(integration_ranks_means$Method)), 1),
        range = c(0.1, 1.0),
        trans = "reverse", breaks = scales::breaks_extended(n = 6)
    ) +
    facet_wrap(~ Integration, nrow = 1) +
    labs(
        title = "Mean ranks",
...
## Dataset scores and ranks

```{r extended-scores-ranks, fig.width = 8, out.width = "100%"}
category_scores_plot <- ggplot(
    ...
    geom_point(aes(colour = Value), shape = "square", size = 1) +
    scale_y_discrete(drop = FALSE) +
    colorspace::scale_colour_continuous_divergingx(
        palette = "Zissou 1",
        limits = c(
            -max(abs(metrics_summary_plotting$Value)),
            max(abs(metrics_summary_plotting$Value))
        )
    ) +
    facet_grid(. ~ Type) +
    labs(
        title = "Metric category scores",
        colour = "Metric category score"
    ) +
    theme_features_pub() +
    theme(
        legend.position = "bottom",
...
category_ranks_plot <- ggplot(
    metric_ranks,
    aes(x = Dataset, y = Method, colour = Type)
) +
    geom_linerange(
        data = methods_meta |>
            mutate(
    ...
    geom_point(
        aes(alpha = length(unique(Method)) -  Rank),
        shape = "square", size = 1
    ) +
    scale_y_discrete(drop = FALSE) +
    scale_colour_manual(values = types_palette, guide = "none") +
    scale_alpha_continuous(
        limits = c(length(unique(metric_ranks$Method)), 1),
        range = c(0.1, 1.0),
        trans = "reverse",
    ...
    facet_grid(. ~ Type) +
    labs(
        title = "Metric category ranks",
        alpha = "Mean category rank",
    ) +
    guides(
        alpha = guide_legend(
            theme = theme(legend.text.position = "bottom"),
...
category_scores_ranks_plot <- wrap_plots(
...
category_scores_ranks_plot
```

## Overlaps means and SDs

```{r extended-overlaps-means-sds, fig.width = 8, out.width = "100%"}
overlaps_means_plot <- ggplot(
    overlaps_plotting,
...
overlaps_sds_plot <- ggplot(
    overlaps_plotting,
    aes(x = Method1, y = Method2, fill = JaccardSD)
) +
    geom_tile() +
...
overlaps_means_sd_plot <- wrap_plots(
    overlaps_means_plot, overlaps_sds_plot,
    nrow = 1,
    widths = c(1, 1)
)

overlaps_means_sd_plot
```

## Dataset overlaps

```{r extended-dataset-overlaps, fig.width = 8, out.width = "100%"}
...
```{r extended-integration-means}
theme_features_integration_extended <- theme_features_integration +
    theme(
        axis.text.x = element_text(size = 5, angle = 90, hjust = 1, vjust = 0.5),
        axis.text.y = element_text(size = 5),
    )

integration_scores_means_plot <- ggplot(
    integration_summary_plotting,
    aes(x = Type, y = Method, colour = Mean)
) +
    geom_point(shape = "square") +
    scale_colour_viridis_c(
        option = "magma",
    ...
    facet_wrap(~ Integration, nrow = 1) +
    theme_features_integration_extended +
    theme(
        axis.ticks.y = element_line(size = 0.25)
    )

integration_scores_diffs_means_plot <- ggplot(
    integration_diff_means_plotting,
    aes(x = Type, y = Method, colour = Mean)
) +
    geom_point(shape = "square") +
...
integration_ranks_means_plot <- ggplot(
    integration_ranks_means_plotting,
    aes(x = Type, y = Method, colour = Type)
) +
    geom_point(
        aes(alpha = length(unique(Method)) -  MeanRank),
        shape = "square"
    ) +
    scale_y_discrete(drop = FALSE) +
    scale_colour_manual(values = types_palette, guide = "none") +
...
integration_diffs_ranks_means_plot <- ggplot(
    integration_ranks_diff_means_plotting,
    aes(x = Type, y = Method, colour = Mean)
) +
    geom_point(shape = "square") +
...
integration_means_plot <- wrap_plots(
    integration_scores_means_plot,
    integration_scores_diffs_means_plot,
    integration_ranks_means_plot,
    integration_diffs_ranks_means_plot,
    nrow = 1,
    widths = c(3, 2, 3, 2)
)

integration_means_plot
```

## Integration SDs

```{r extended-integration-sds}
integration_scores_sds <- ggplot(
    integration_summary_plotting,
    aes(x = Type, y = Method, colour = SD)
) +
    geom_point(shape = "square") +
...
integration_scores_diffs_sds <- ggplot(
    integration_diff_means_plotting,
    aes(x = Type, y = Method, colour = SD)
) +
    geom_point(shape = "square") +
...
integration_ranks_sds <- ggplot(
    integration_ranks_means_plotting,
    aes(x = Type, y = Method, colour = SDRank)
) +
    geom_point(shape = "square") +
    scale_y_discrete(drop = FALSE) +
    scale_colour_viridis_c(option = "cividis") +
...
integration_diffs_ranks_sds <- ggplot(
    integration_ranks_diff_means_plotting,
    aes(x = Type, y = Method, colour = SD)
) +
    geom_point(shape = "square") +
...
integration_sds_plot <- wrap_plots(
    integration_scores_sds,
    integration_scores_diffs_sds,
    integration_ranks_sds,
    integration_diffs_ranks_sds,
    nrow = 1,
    widths = c(3, 2, 3, 2)
)

integration_sds_plot
```
...
```{r supplementary-ranks, fig.height = 6, out.width = "100%"}
metric_ranks <- metrics_summary |>
    filter(Integration == "scVI-1") |>
    pivot_longer(
        cols = Classification:Overall,
...
ranks_plot <- ggplot(metric_ranks) +
    ggforce::geom_sina(
        aes(x = length(unique(Method)) - Rank + 1, y = Method),
        size = 0.5, alpha = 0.3
    ) +
    geom_point(
        data = metric_ranks_means,
        aes(
            x = length(unique(Method)) - MeanRank + 1,
            y = Method,
            fill = length(unique(Method)) - MeanRank + 1
        ),
        shape = "circle filled", size = 2
    ) +
    scale_fill_viridis_c(direction = -1) +
    scale_x_reverse(
        limits = c(length(unique(metric_ranks$Method)) + 1, 1),
        breaks = c(1, 10, 20, 30)
    ) +
    facet_grid(. ~ Type, scales = "free_y", space = "free_y") +
    labs(
        title = "Method ranks by metric type",
        x = "Method rank",
        fill = "Mean rank"
    ) +
    theme_features_pub() +
...
```{r supplementary-overlaps, fig.height = 12, out.width = "100%"}
overlaps_all_plotting <- overlaps |>
    mutate(
        Method1 = factor(
            Method1,
            levels = overlaps_clust_order,
...
dataset_overlaps_plot <- ggplot(overlaps_all_plotting) +
    geom_tile(aes(x = Method2, y = Method1, fill = Jaccard)) +
    scale_fill_viridis_c(option = "plasma", limits = c(0, 1)) +
    ...
    labs(
        title = "Overlap in selected features by dataset",
        fill = "Jaccard\nIndex"
    ) +
    theme_features_pub() +
    theme(
...
```{r supplementary-integration-scores, fig.height = 9, out.width = "100%"}
integration_summary_long <- integration_summary |>
    pivot_longer(
        cols = scVI:Symphony,
        names_to = "Integration",
        values_to = "Value"
...
integration_scores_all_plot <- ggplot(integration_summary_long) +
    annotate(
        "rect",
        xmin = -Inf, xmax = 0, ymin = -Inf, ymax = Inf,
        fill = "red", alpha = 0.1
    ...
    geom_point(
        data = integration_summary_plotting,
        aes(x = Mean, y = Method, fill = Mean),
    ...
    labs(
        title = "Metric summary scores by integration",
        x = "Metric type summary score",
        fill = "Mean score",
        caption = paste(
            "Note:", sum(abs(integration_summary_long$Value) > 3),
            "values outside the range [-3, 3] are not shown"
        )
    ) +
    theme_features_pub() +
    theme(
...
integration_scores_all_plot
```

## Integration scores differences

```{r supplementary-integration-scores-diffs, fig.height = 6, out.width = "100%"}
integration_diffs_long <- integration_diffs |>
    select(-scVI, -scANVI, -Symphony) |>
    pivot_longer(
        cols = scANVIDiff:SymphonyDiff,
        names_to = "Integration",
        values_to = "Difference"
    ) |>
    mutate(
        Integration = str_remove(Integration, "Diff")
    ) |>
    mutate(
        Integration = factor(
            Integration,
            levels = c("scANVI", "Symphony")
        ),
        Method = factor(
            Method,
            levels = methods_order,
...
integration_diffs_all_plot <- ggplot(integration_diffs_long) +
    geom_vline(xintercept = 0, colour = "red") +
    ggforce::geom_sina(
        aes(x = Difference, y = Method),
        size = 1, alpha = 0.3
    ) +
    geom_point(
        data = integration_diff_means_plotting,
        aes(x = Mean, y = Method, fill = Mean),
        shape = "circle filled", size = 1.5
    ) +
    colorspace::scale_fill_continuous_diverging(
    ...
    facet_grid(Integration ~ Type) +
    labs(
        title = "Difference in metric summary scores by integration",
        x = "Difference in metric type summary score",
        fill = "Mean difference",
        caption = paste(
            "Note:", sum(abs(integration_diffs_long$Difference) > 3),
            "values outside the range [-3, 3] are not shown"
        )
    ) +
    theme_features_pub() +
...
integration_diffs_all_plot
```

## Integration ranks

```{r supplementary-integration-ranks, fig.height = 9, out.width = "100%"}
integration_ranks_plotting <- integration_ranks |>
    mutate(
        Integration = factor(
            Integration,
            levels = c("scVI-1", "scANVI-1", "Symphony-1"),
            labels = c("scVI", "scANVI", "Symphony")
        ),
        Method = factor(
            Method,
            levels = methods_order,
...
integration_ranks_all_plot <- ggplot(integration_ranks_plotting) +
    ggforce::geom_sina(
        aes(
            x = length(unique(integration_ranks$Method)) - Rank + 1,
            y = Method
        ),
        size = 1, alpha = 0.3
    ) +
    geom_point(
        data = integration_ranks_means_plotting,
        aes(
            x = length(unique(integration_ranks$Method)) - MeanRank + 1,
            y = Method,
            fill = MeanRank
        ),
        shape = "circle filled", size = 1.5
    ...
    scale_fill_viridis_c() +
    scale_x_reverse(
        limits = c(length(unique(integration_ranks$Method)) + 1, 0),
        breaks = c(1, 10, 20, 30)
    ) +
    facet_grid(Integration ~ Type) +
    labs(
        title = "Metric summary score ranks by integration",
        x = "Metric type summary score rank",
        fill = "Mean rank",
    ) +
    theme_features_pub() +
    theme(
...
integration_ranks_all_plot
```

## Integration ranks differences

```{r supplementary-integration-ranks-diffs, fig.height = 6, out.width = "100%"}
integration_rank_diffs_long <- integration_rank_diffs |>
    select(-scVI, -scANVI, -Symphony) |>
    pivot_longer(
        cols = scANVIDiff:SymphonyDiff,
        names_to = "Integration",
...
integration_rank_diffs_all_plot <- ggplot(integration_rank_diffs_long) +
    geom_vline(xintercept = 0, colour = "red") +
    ggforce::geom_sina(
        aes(x = Difference, y = Method),
        size = 1, alpha = 0.3
    ...
    geom_point(
        data = integration_ranks_diff_means_plotting,
        aes(x = Mean, y = Method, fill = Mean),
        shape = "circle filled", size = 1.5
    ) +
    colorspace::scale_fill_continuous_diverging(
    ...
    facet_grid(Integration ~ Type) +
    labs(
        title = "Difference in metric summary score ranks by integration",
        x = "Difference in metric type summary score rank",
        fill = "Mean difference",
    ) +
    theme_features_pub() +
    theme(
...
integration_rank_diffs_all_plot
```
...
save_figure_files(
    overlaps_panel,
    here::here("analysis", "output", "benchmark-overlaps"),
    width = 7, height = 3
)
...
save_figure_files(
    category_scores_ranks_plot,
    here::here("analysis", "output", "category-scores-ranks"),
    width = 7, height = 5.5
)

save_figure_files(
    overlaps_means_sd_plot,
    here::here("analysis", "output", "overlaps-means-sds"),
    width = 7, height = 5
)

save_figure_files(
    dataset_overlaps_plot,
    here::here("analysis", "output", "dataset-overlaps"),
    height = 2.5
)

save_figure_files(
    integration_means_plot,
    here::here("analysis", "output", "integration-means"),
    width = 7, height = 4
)

save_figure_files(
    integration_sds_plot,
    here::here("analysis", "output", "integration-sds"),
    width = 7, height = 4
)

save_figure_files(
    ranks_plot,
    here::here("analysis", "output", "metric-ranks"),
    height = 6
)

save_figure_files(
    integration_scores_all_plot,
    here::here("analysis", "output", "integration-scores"),
    height = 9
)

save_figure_files(
    integration_diffs_all_plot,
    here::here("analysis", "output", "integration-score-diffs"),
    height = 6
)

save_figure_files(
    integration_ranks_all_plot,
    here::here("analysis", "output", "integration-ranks"),
    height = 9
)

save_figure_files(
    integration_rank_diffs_all_plot,
    here::here("analysis", "output", "integration-rank-diffs"),
    height = 6
)
```
